...
CHAR(n)      : 고정 길이 문자열 한 글자씩 최대 몇 글자 작성할 것인지 설정
               작성을 안 했을 경우 1byte로 컬럼하나에 오직 숫자나 영어 한 글자만 가능
VARCHAR(n)   : Oracle에서는 사용 불가 표준 SQL에서는 사용 가능
VARCHAR2(n)  : VARCHAR보다 더 큰 공간을 사용할 수 있음
...
NUMBER(p, s) : 숫자나 실수를 담는 타입
               ...
               Number(38,0) → 최대 38자리 정수와 소수점 밑 실수 하나라도 작성 XXX
