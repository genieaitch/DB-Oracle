...
-- 		   CREATE SEQUENCE 가져간다 START WITH 700 INCREMENT BY1;
--			가져가는 사람 수를 700 번부터 700, 701, 702, ... 순차적으로 가져간 인원 체크

CREATE SEQUENCE seq_user; -- START WITH 1 INCREMENT BY 1이 기본값으로 설정

-- 테이블 생성 USER 예약어이기 때문에 TABLE 명으로 작성할 경우 ``이나 ""로 만들기
...
FOR EACH ROW
BEGIN
	:NEW.studentid := seq_user.NEXTNAL;
END;
/* / ORACLE 구문할 때  명령프롬포트(CMD)나 SQLDEVELOPER 환경에서 사용할 떄는 작성해야함*/

INSERT INTO STUDENT(STUDENTID, STUDENTNAME, STUDENTAGE)
			VALUES(seq_user.NEXTNAL, '안정미', 15);
