...
COMMIT; --INSERT UPDATE DELETE 추가 수정 삭제 한 기록 저장하기(COMMIT)

ROLLBACK; -- DBEAVER는 알아서 자동 COMMIT함

DELETE FROM EATS WHERE CAFE_ID = 20;

-- 2번 방법을 이용해서 선택적으로 데이터 추가
---- 주의할 점 : NOT NULL은 선택적으로 작성 불가 필수로 작성해야하는 컬럼
---- PRIMARY KEY → 고유의 숫자값을 가지는 키 / 가입한 명단 순서 NULL
--- 트랜잭션 시퀀스 → ORACLE PRIMARY KEY는 자동으로 번호를 측정해주는 동작을 설정
INSERT INTO EATS E(CAFE_ID, CAFE_NAME) VALUES (15, '빈포레스트');

-- 빈 포레스트라는 CAFE_NAME을 찾아서 삭제
DELETE FROM EATS E WHERE CAFE_NAME = '빈포레스트';

-- INSERT INTO EATS(CAFE_ID, CAFE_NAME, LOCATION, CONTACT_NUMBER)
-- VALUE()

INSERT INTO EATS E(CAFE_ID, CAFE_NAME, LOCATION, CONTACT_NUMBER)
...
INSERT INTO EATS_MENU EM (MENU_ID,CAFE_ID,MENU_NAME,DESCRIPTION,PRICE)
