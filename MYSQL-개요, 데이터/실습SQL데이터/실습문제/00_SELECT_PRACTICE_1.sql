/*
USE DATABASE; 명칭은 한 회사에서 여러 서비스를 제공하고 각 서비스에 따른 데이터베이스에 접속할 때 사용하는 기능
...
2. DEUM 데이터베이스에 접속해서 회원정보나 게시글 관리, 테이블 확인하길 원한다면
   USE DEUM;
   SELECT*FROM DEUM 회원정보테이블;
   
   MALON 데이터베이스 접속해서 회원정보, 음악목록 테이블 조회하길 원한다면
   USE MALON;
...
-- 유저가 보고자하는 데이터베이스가 존재하지 않을 떄 발생하는 Error Code: 1146. Table 'khtdb.cafe' doesn't exist

CREATE TABLE USER (
    user_id INT PRIMARY KEY auto_increment,  -- 유저의 가입순서
    user_name VARCHAR(100) NOT NULL,  -- 유저 이름
    location VARCHAR(255),            -- 유저 위치
...
insert INTO USER (USER_NAME, LOCATION, CONTACT_NUMBER)
 ...
 -- SELECT INSERT UPDATE와 같은 QUERY 작업을 진행할 때
...
/*
-- USER_ID 기본키 자동증가
-- USERNAME 문자열 빈 값 못들어가게 설정 (50 BYTE)
...
-- khtdb에 userprofile 이라는 테이블이 존재하지 않으므로 create table userprofile을 이용해서alter
...
-- 테이블 삭제 * 주의 테이블 삭제 함부로 하지 말 것 반드시 혼자하더라도 상의 후 삭제할 것
-- 테이블 삭제 = DROP TABLE 테이블 내 데이터 삭제
-- DELETE FROM 테이블명 WHERE = 테이블에서 하나의 행 삭제할 조건;
-- Error Code: 1051. Unkonwn table 'khtdb.userprofile' 텡디블이 존재하지 않기 때문에 삭제 불가
DROP TABLE USERPROFILE;
DELETE FORM USERPROFILE; -- 테이블 내 모든 데이터 삭제 하나의 데이터만 삭제하길 원한다면 WHERE