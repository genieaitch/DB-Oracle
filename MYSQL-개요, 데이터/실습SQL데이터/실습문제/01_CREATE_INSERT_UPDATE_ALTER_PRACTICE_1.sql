...
-- KHTUSER 데이터 베이스에서 USER TABLE 홍길동의 이메일주소와 전화번호 조회
USE KHTUSER;
SELECT EMAIL, PHONE
FROM USER
...
-- CAFE ORDER 테이블에서 SATUS가 PREPARING 인 주문의 테이블 번호와 주문 금액 조회
...
-- UPDATE문을 SQL에서 직접적으로 실행할 때 문제가 되는 것을 방지하기 위해 SAFE 모드 생성
...
MODIFY COLUMN STATUS ENUM('준비 중', '완료', '대기 중', '취소');
