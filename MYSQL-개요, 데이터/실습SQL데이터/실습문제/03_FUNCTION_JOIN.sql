...
select * FROM REVIEW;
SELECT LENGTH(COMMENT) FROM REVIEW WHERE PRODUCT_ID LIKE '3';
-- → 총 길이 66 왜냐하면 한글은 한 글자 당 크기가 = 3 영어 숫자 이외 모든 한 글자 크기 =3
-- → 한글 20자 띄어쓰기 5개 마침표 1개

-- REVIEW 제품아이디 4번에 작성된 댓글에서 좋고 → 훌륭하고 바꿔서 출력
SELECT REPLACE(COMMENT,'좋고', '훌륭하고') FROM REVIEW WHERE PRODUCT_ID LIKE '4';
...
-- REVIEW 테이블과 ORDER 테이블을 RIGHT JOIN으로 연결해서 모든 리뷰가 포함되도록 하고, 주문정보가 없는 경우 NULL 표기되게 하기
...
-- LEFT JOIN
SELECT S.ID, S.NAME, S.CATEGORY, S.PRICE, S.STOCK, O.QUANTITY, O.ORDER_DATE, O.STATUS
FROM SNACK S
