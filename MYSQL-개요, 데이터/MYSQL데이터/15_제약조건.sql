...
PRIMARY KEY(기본키)
 ...
 - 기본 키로 설정된 컬럼은 자동으로 UNIQUE와 NOT NULL 제약 조건을 가짐
   ...
   12월 10일에 가입한 회원의 번호가 13번이라면
   12월 11일에 가입한 회원의 번호가 14번이거나 13번 이후 번호가 되어야지
   회원가입 순번이 13이거나 빈 값이면 안된다는 의미
   ...
   테이블_ID 컬럼 PRIMARY 키이며 각 키 값이 고유하고, NULL(빈) 값을 가질 수 없음
...
[작성법]
   ...
   유저명과 가입일자 모두 NULL(빈) 값 작성 XX
   →
   유저명 : 회원가입 시 유저가 유저명 작성하지 않으면 회원가입 X
   가입일자 : 회원가입 시 유저가 회원가입일자를 작성하지 않아도 DB에 회원가입이 저장되는 시간으로 자동 기록
...
[작성법]
   ...
   마찬가지로 주민번호, 휴대폰번호 또한 다른 유저와 일치하는 번호값이 없어야함
   ...
   2 lee@googla.co.kr 000101-0202020 주민번호와 이메일 모두 일치하는 회원이 없기 때문에 저장 가능
...
FOREIGN KEY
 ...
 - 참조 무결성 유지
 - 부모 테이블의 데이터 변경 시 행동(삭제/ 없데이트) 제어할 수 있음
...
[작성법]
   ...
   ON DELETE CASCADE : 만약 다른테이블에서 참조하는 id에서 delete(삭제)가 발생하면
   ...
   예를 들어 ) 커뮤니티 카페의 경우 카페 회원 탈퇴를 할 경우 회원이 작성한 게시물과 댓글 삭제X
