...
1) OR REPLACE 옵션 : --ORACLE에서만 사용 가능 MYSQL(5.7 버전 이상만 가능 = 우리는 가능)
                기존에 동일한 이름의 VIEW가 존재하면 덮어쓰기처럼 현재 작성한 VIEW로 변경
...
-- VIEW 테이블은 JOIN해서 만든 테이블일 경우 INSERT UPDATE DELETE 가 안됨
-- 단일 테이블로 VIEW를 설정하면 INSERT DELETE가 임시적으로는 가능하지만
...
시퀀스명.NEXTNAL : 다음 시퀀스 번호를 얻어옴 (INCREMENT BY 만큼 증가된 수)
                  단, 생성 후 처음 호출된 시퀀스인 경우 START WITH에 작성된 값이 반환됨

시퀀스명.CURRVAL : 현재 시퀀스 번호를 얻어옴
                  단, 시퀀스가 생성 되자마자 호출할 경우 오류 발생
                  마지막으로 호출한 NEXTNAL 값을 반환

MYSQL - SEQUENCE 지원 XXXX AUTO_INCREMENT를 사용
*/
...
- 데이터 변경(INSERT, UPDATE, DELETE) 작업 시 이진 트리 구조에 변형이 일어남
    → DML 작업이 번번한 경우 시스템 부하가 늘어 성능이 저하됨
...
** 인덱스가 자동으로 생성되는 경우 **
 → PX 또는 UNIQUE 제약조건이 설정된 컬럼에 대해 UNIQUE INDEX가 자동 생성된다.
