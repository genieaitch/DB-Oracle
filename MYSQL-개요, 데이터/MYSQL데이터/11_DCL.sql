...
사용자 계정 : 데이터베이스에 대해서 질의, 갱신, 보고서 작성 등의 작업을 수행할 수 있는 계정으로
...
/*
DCL (Data Control Language) : 계정에 DB, DB 객체에 대한접근 권한을 부여하고 회수하는 언어
...
--            192.168.0.1(현재 네트워크 내 존재하는 컴퓨터) : 우리집 거실에 있는 컴퓨터
--            211.0.0.1(특정 ip 주소에서만 접근 가능) : 개똥이네에서 우리집 DB 접속 가능
--            → ip 주소 확인하는 방법 cmd → ipconfig / 특정 주소는 검색 또는 공유기에서 확인 가능


-- 계정 OWNER / NEWBIE

-- 예를 들어 KHCAFE / KHUSER 데이터베이스 존재하고, 각 직급에 따라 데이터베이스 적속 권한 부여

GRANT ALL PRIVILEGES ON KHCAFE.menu TO 'NEWBIE'@'%';
--→ 새로운 신입 계정은 KHCAFE 데이터베이스에서 menu 테이블만 접근 가능

GRANT ALL PRIVILEGES ON KHUSER.* TO 'OWNER'@'%';
...
-- JUNIOR 가 이상한 행동을 보이는 게 맞음! 혹시 모르니 모든 권한 회수
